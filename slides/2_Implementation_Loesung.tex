...
erweitert.\newline
Der SoC erstellt Speicherregionen, Peripheriegeräte und implementiert\newline
eine spezifische Mikroprozessor Architektur.\newline
Die Auswahl des SoC erfolgt mittels einer Maschine.
\end{block}
...
Analyse und Identifikation der bereits in QEMU integrierten Peripherie.\newline
\textbf{Existiert bereits:}
\begin{itemize}
    \item SysCfg und EXTI (STM32F4)
    \item Timer, ADC und SPI (STM32F2)
    \item USART (STM32F2) \rightarrow\hspace{1mm} ausgewählt für Tests
\end{itemize}
\textbf{Existiert noch nicht:}
\begin{itemize}
    \item GPIO Controller \rightarrow\hspace{1mm} ausgewählt für selbständige
        Implementierung
    \item Ethernet, CAN, RCC, etc.
\end{itemize}
...
\begin{itemize}
    ...
    \item Erweiterung der Build Konfigurationen
    \item Implementierung von Testmaschine und SoC
    \item Implementierung der Peripherie
\end{itemize}
